...
class DataBase:
    ...
    async def insert_all_data(self):
        ...
        lots = [
            Lot(lot.get("url"), lot.get("description"), lot.get("price"), self.str_date()) for lot in data
        ]
    ...
    async def insert_new_lots(self) -> list[Lot]:
        ...
        new_lots = [
            Lot(url=lot.get("url"), description=lot.get("description"), price=lot.get("price"), created=self.str_date())
            for lot in lots
    ...
    async def delete_old_lots(self):
        ...
        async with aiosqlite.connect(self.name) as conn:
            ...
            await conn.executemany('DELETE FROM lots WHERE url = ?',
                                   old_lot_urls)
            await conn.commit()
